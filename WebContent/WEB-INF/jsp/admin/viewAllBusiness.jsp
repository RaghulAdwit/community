...
<!-- admin_viewAllBusiness starts -->
<link href="<%=assetsPath %>/admin/layout2/css/admin_viewAllbusiness.css"
	rel="stylesheet" type="text/css" />
<!-- admin_viewAllBusiness ends -->
<style>
</style>
</head>
		...
		<div class="page-content-wrapper">
			...
			<div>
				...
				<div class="top_head_txt">
					<b><p class="padding-top-10">Manage Business</p></b>
				</div>
			</div>
			<!-- END PAGE HEADER-->
			...
			<div class="page-content ">
				...
				<p class="black">
					<b><a href="<%=contextPath %>/admin/business/add">Add New</a> </b>
				</p>
							...
							<div class="portlet-body back_white">
								...
								for(Advertiser advertiser : advertiserList){ %>
										<tr class="odd <%=index%2==0?"gradeX":""%>">
											...
											<td><%=advertiser.getName() %></td>
											<td><%=advertiser.getCategory().getName() %></td>
											<td><%=advertiser.isFeatured()?"Yes":"No" %></td>
											<td><a
												href="<%=contextPath %>/admin/business/edit/<%=CipherUtils.encryptId(advertiser.getAdvertiserId()+"") %>/"><span
													class="glyphicon glyphicon-edit"> </span></a> <a
												href="<%=contextPath %>/admin/business/gallery/<%=CipherUtils.encryptId(advertiser.getAdvertiserId()+"") %>/"><span
													class="glyphicon glyphicon-picture"> </span></a> <a
												href="javascript:;"
												onclick="removeBusiness('<%=advertiser.getAdvertiserId() %>');"><span
...
removeBusiness = function(advertiserId){
var result = confirm("Are you sure want to delete the business detail?");
	
	if(result){
		$.ajax({  
		       url : '<%=contextPath%>/admin/business/remove',  
		       type : 'POST',
		       dataType: 'json',
