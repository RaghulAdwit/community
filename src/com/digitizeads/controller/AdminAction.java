...
import com.digitizeads.modal.AdvertiserView;
...
public class AdminAction {  
	...
	public ModelAndView displayAdminHome(HttpServletRequest request, HttpServletResponse response) {
		...
		User admin = session.getAttribute(Constants.ADMIN_DTO) != null
				? (User) session.getAttribute(Constants.ADMIN_DTO)
				: null;
	...
	public ModelAndView executeAddBusiness(HttpServletRequest request, HttpServletResponse response,
		...
		if (admin != null && admin.getUserId() > 0) {
			...
			String featuredVideo = request.getParameter("youtubelink");

			System.out.println("youtube link :" + featuredVideo);
			...
			advertiser.setFeaturedVideo(featuredVideo);
...
//					UploadFile s3Folder = new UploadFile();
					...
					// s3Folder.addBusinessImage(destinationFile, convFile,
					// s3Folder.getAmazonS3Client());
	...
	@RequestMapping(value = "/subdomain/add", method = RequestMethod.GET)
	public ModelAndView displayAddSubdomain(HttpServletRequest request, HttpServletResponse response) {
		HttpSession session = request.getSession();
		ModelAndView model = new ModelAndView("/admin/login");
		...
		if (admin != null && admin.getUserId() > 0) {
			ApplicationContext context = new ClassPathXmlApplicationContext("Beans.xml");
			model = new ModelAndView("/admin/addsubdomain");
		}

		return model;
	}

	@RequestMapping(value = "/subdomain/add", method = RequestMethod.POST)
	public ModelAndView executeAddSubdomain(HttpServletRequest request, HttpServletResponse response) {
		HttpSession session = request.getSession();
		ModelAndView model = new ModelAndView("/admin/login");
		...
		if (admin != null && admin.getUserId() > 0) {
			...
			String subdomainLink = request.getParameter("subdomainLink");
			String bannerimgLink = request.getParameter("bannerimgLink");
			...
			subDomain.setLogoUrl(logoLink);
			subDomain.setSubdomainURL(subdomainLink);
			subDomain.setBannerImageURL(bannerimgLink);
			subDomain.setHeaderText(header);
			subDomain.setFooterText(footer);

			int result = adminBean.addSubdomainDetails(subDomain);

			if (result > 0) {
				request.setAttribute(Constants.ADMIN_SUCCESS_MESSAGE, "SubDomain Details added.");
			} else if (result == -2) {
				request.setAttribute(Constants.ADMIN_ERROR_MESSAGE,
						"SubDomain Details already exist with the same name.");
			} else {
				request.setAttribute(Constants.ADMIN_ERROR_MESSAGE, "Error while adding SubDomain Details.");
			}
...
//		<% } %>
	...
	@RequestMapping(value = "/subdomain/viewall", method = RequestMethod.GET)
	public ModelAndView displayViewAllSubdomain(HttpServletRequest request, HttpServletResponse response) {
		HttpSession session = request.getSession();
		ModelAndView model = new ModelAndView("/admin/login");
		String contextPath = request.getContextPath();
		...
		if (admin != null && admin.getUserId() > 0) {
			...
			List<SubDomain> subDomainList = adminBean.getSubDomainList();
			request.setAttribute(Constants.SUBDOMAIN_LIST, subDomainList);

			model = new ModelAndView("/admin/viewAllSubDomain");
		}

		return model;
	}

	@RequestMapping(value = "/subDomain/edit/{subdomain_id}", method = RequestMethod.GET)
	public ModelAndView displaysubdomainDetails(HttpServletRequest request, HttpServletResponse response,
			@PathVariable("subdomain_id") String subDomainId) {
		System.out.println(subDomainId);
		HttpSession session = request.getSession();
		ModelAndView model = new ModelAndView("/admin/login");
		...
		if (admin != null && admin.getUserId() > 0) {
			...
			subDomainId = CipherUtils.decryptId(subDomainId);
			System.out.println(subDomainId);
			SubDomain subDomain = adminBean.getSubDomainDetails(Integer.parseInt(subDomainId));
			System.out.println(subDomain);
			request.setAttribute(Constants.SUBDOMAIN_DETAILS, subDomain);
			model = new ModelAndView("/admin/editSubDomain");
		}

		return model;
	}

	@RequestMapping(value = "/subDomain/edit/{subdomain_id}", method = RequestMethod.POST)
	public ModelAndView editsubDomainDetails(HttpServletRequest request, HttpServletResponse response,
			@PathVariable("subdomain_id") String subDomainId) {
		HttpSession session = request.getSession();
		ModelAndView model = new ModelAndView("/admin/login");
		...
		if (admin != null && admin.getUserId() > 0) {
			...
			subDomain.setHeaderText(header);
			subDomain.setFooterText(footer);
			subDomain.setSubDomainId(Integer.parseInt(subDomainId));

			int result = adminBean.editSubDomainDetails(subDomain);

			if (result > 0) {
				request.setAttribute(Constants.ADMIN_SUCCESS_MESSAGE, "SubDomain Details edited.");
			} else if (result == -2) {
				request.setAttribute(Constants.ADMIN_ERROR_MESSAGE,
						"SubDomain Details  already exist with the same name.");
			} else {
				request.setAttribute(Constants.ADMIN_ERROR_MESSAGE, "Error while editing SubDomain Details .");
			}

			SubDomain subDomainDetails = adminBean.getSubDomainDetails(Integer.parseInt(subDomainId));
			request.setAttribute(Constants.SUBDOMAIN_DETAILS, subDomain);
	...
	public ModelAndView displayReports(HttpServletRequest request, HttpServletResponse response) {
		HttpSession session = request.getSession();
		ModelAndView model = new ModelAndView("/admin/login");
		...
		if (admin != null && admin.getUserId() > 0) {
			...
			model = new ModelAndView("/admin/viewreport");
		}
		return model;
	}
	...
	public ModelAndView executeReports(HttpServletRequest request, HttpServletResponse response) {
		...
		if (admin != null && admin.getUserId() > 0) {
			...
			String advertiserId = request.getParameter("advertiser");
			String advertiserText = request.getParameter("advertiserText");   

			List<Advertiser> advertiserList = null;
			List<SubDomain> subDomainList = null;
			List<AdvertiserTrack> advertiserTrackList = null;
			int views = 0;
			System.out.println("**************************");
			...
			if (!Constants.isStringNull(subdomainId)) {
				advertiserList = adminBean.getAdvertiserIdName(Integer.parseInt(subdomainId));
				request.setAttribute("subdomainId", subdomainId);				  
			}
			
			subDomainList = adminBean.getSubDomainList();
			request.setAttribute(Constants.SUBDOMAIN_LIST, subDomainList);  
			request.setAttribute(Constants.ADMIN_ADVERTISER_LIST, advertiserList);
			
			int tempSubDomainId = 0;
			if (!Constants.isStringNull(advertiserId)) {
				System.out.println("checked");
				tempSubDomainId = adminBean.checkSubDomain(Integer.parseInt(advertiserId));
				System.out.println("TempSubDomainId" + tempSubDomainId);
				if (tempSubDomainId != Integer.parseInt(subdomainId)) {
					advertiserTrackList = null;
					model = new ModelAndView("/admin/viewreport");
					return model;  
				}
			}
			...
			if (!Constants.isStringNull(advertiserId)) {
				advertiserTrackList = adminBean.getAdvertiserTrack(Integer.parseInt(advertiserId));
				request.setAttribute(Constants.ADVERTISER_TRACK, advertiserTrackList);
				views = adminBean.getAdvertiserView(Integer.parseInt(advertiserId));
				request.setAttribute("advertiserId", advertiserId);
				request.setAttribute("views", views);
				System.out.println(advertiserTrackList);
				System.out.println(views);	
			}


			System.out.println("advertiserlist"+advertiserList);
			System.out.println("**************************");
