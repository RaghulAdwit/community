...
import com.digitizeads.mapper.AdvertiserMapper;
...
public class HomeAction {
	...
	public ModelAndView displayHome(HttpServletRequest request, HttpServletResponse response) {
		...
		SubDomain userSubDomain = session.getAttribute(Constants.USER_SUBDOMAIN) != null
				? (SubDomain) session.getAttribute(Constants.USER_SUBDOMAIN)
				: null;
	...
	public ModelAndView displayBusinessDetails(HttpServletRequest request, HttpServletResponse response,
		...
		System.out.println("FeaturedVideo :" + advertiserDetails.getFeaturedVideo());
	...
	@RequestMapping(value = "/test/{business_slug}", method = RequestMethod.GET)
	public @ResponseBody Advertiser test(HttpServletRequest request, HttpServletResponse response,
			@PathVariable("business_slug") String businessSlug) {
		System.out.println(businessSlug+"hi");
		Advertiser advertiser = null;
		String query = "SELECT * WHERE STATUS=1 AND URL_SLUG=?";

		advertiser = jdbcTemplate.queryForObject(query, new Object[] { businessSlug }, new AdvertiserMapper());
		return advertiser;
