...
public class AdminBean {
	...
	public int addBusinessDetails(Advertiser advertiser) {
		...
		String INSERT_USER_SESSION = "INSERT INTO  t_advertiser (NAME, LOGO, CATEGORY_ID, LATITUDE, LONGITUDE, ADDRESS, PHONE_NO,"
				...
				+ " SUBDOMAIN_ID, URL_SLUG, FEATURED_VIDEO)  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?, ?, ?, ?, ?, ?, ?)";
		...
		Object[] params = new Object[] { advertiser.getName(), advertiser.getLogo(),
				...
				advertiser.getSubDomain().getSubDomainId(), advertiser.getUrlSlug(), advertiser.getFeaturedVideo() };
	...
	public int editBusinessDetails(Advertiser advertiser) {
		...
		String INSERT_USER_SESSION = "UPDATE t_advertiser SET NAME=?, LOGO=?, CATEGORY_ID=?, LATITUDE=?, LONGITUDE=?, ADDRESS=?, PHONE_NO=?,"
				+ " EMAIL=?, WEBSITE=?, ABOUT_US=?, IS_FEATURED=?, FB_LINK=?, TWITTER_LINK=?, INSTAGRAM_HANDLE=?,FEATURED_VIDEO=? WHERE ADVERTISER_ID=?";

		Object[] params = new Object[] { advertiser.getName(), advertiser.getLogo(),
				advertiser.getCategory().getCategoryId(), advertiser.getLatitude(), advertiser.getLongitude(),
				advertiser.getAddress(), advertiser.getPhoneNumber(), advertiser.getEmail(), advertiser.getWebsite(),
				advertiser.getAboutUs(), advertiser.isFeatured(), advertiser.getFbLink(), advertiser.getTwitterLink(),
				advertiser.getInstagramHandleName(), advertiser.getFeaturedVideo(), advertiser.getAdvertiserId() };
	...
	public Advertiser getAdvertiserDetails(int advertiserId) {
		...
		String query = "SELECT TA.ADVERTISER_ID, TA.NAME, TA.LOGO, TA.CATEGORY_ID, TA.LATITUDE, TA.LONGITUDE, TA.ADDRESS, TA.PHONE_NO, TA.EMAIL,"
				...
				+ " TC.IMAGE AS CAT_IMAGE, TA.FB_LINK, TA.TWITTER_LINK, TA.INSTAGRAM_HANDLE, TA.FEATURED_VIDEO FROM t_advertiser TA LEFT JOIN "
	...
	public ArrayList<SubDomain> getSubDomainList() {
		...
		String query = "SELECT SUBDOMAIN_ID, NAME, LOGO_URL, SUBDOMAIN_URL, FOOTER_TEXT, HEADER_TEXT, STATUS, BANNER_IMG_URL FROM t_subdomain WHERE"
	...
	public int addSubdomainDetails(SubDomain subDomain) {
		String query = "SELECT SUBDOMAIN_ID FROM t_subdomain WHERE NAME=? LIMIT 1";

		Object[] conditionObject = new Object[] { subDomain.getName() };
		...
		String INSERT_USER_SESSION = "INSERT INTO t_subdomain (NAME, LOGO_URL, SUBDOMAIN_URL, FOOTER_TEXT, HEADER_TEXT, BANNER_IMG_URL) VALUES(?,?,?,?,?,?)";
		Object[] params = new Object[] { subDomain.getName(), subDomain.getLogoUrl(), subDomain.getSubdomainURL(),
				subDomain.getHeaderText(), subDomain.getFooterText(), subDomain.getBannerImageURL() };

		return jdbcTemplate.update(INSERT_USER_SESSION, params);
	}

	public int editSubDomainDetails(SubDomain subDomain) {
		String query = "SELECT SUBDOMAIN_ID FROM t_subdomain WHERE NAME=? LIMIT 1";
		Object[] conditionObject = new Object[] { subDomain.getName(), subDomain.getSubDomainId() };
		...
		String INSERT_USER_SESSION = "UPDATE t_subdomain SET NAME=?, LOGO_URL=?, SUBDOMAIN_URL=?, FOOTER_TEXT=?, HEADER_TEXT=?, BANNER_IMG_URL=? WHERE SUBDOMAIN_ID=?";
	...
	public List<Advertiser> getAdvertiserIdName(int subDomainId) {

		String query = "";
		List<Advertiser> advertiserList = null;
		try {
			query = "SELECT ADVERTISER_ID, NAME FROM adwit_digitized_ads.t_advertiser where status=1 and SUBDOMAIN_ID=? order by name;";

			Object[] params = new Object[] { subDomainId };

			advertiserList = jdbcTemplate.query(query, params, new RowMapper<Advertiser>() {
				public Advertiser mapRow(ResultSet rs, int rowNum) throws SQLException {
					Advertiser advertiser = new Advertiser();
					advertiser.setAdvertiserId(rs.getInt("ADVERTISER_ID"));
					advertiser.setName(rs.getString("NAME"));
	...
	public List<AdvertiserTrack> getAdvertiserTrack(int advertiserId) {
		String query = "";
		List<AdvertiserTrack> advertiserTrackList = null;
		try {
			query = "SELECT count(SOURCE_TYPE), SOURCE_TYPE FROM adwit_digitized_ads.t_track_advertiser_click where ADVERTISER_ID=? group by SOURCE_TYPE";

			Object[] params = new Object[] { advertiserId };

			advertiserTrackList = jdbcTemplate.query(query, params, new RowMapper<AdvertiserTrack>() {
				public AdvertiserTrack mapRow(ResultSet rs, int rowNum) throws SQLException {
					AdvertiserTrack advertiserTrack = new AdvertiserTrack();
					advertiserTrack.setSourceType(rs.getString("SOURCE_TYPE"));
					advertiserTrack.setCount(rs.getInt("count(SOURCE_TYPE)"));
					return advertiserTrack;
				}
			});
			return advertiserTrackList;
		} catch (EmptyResultDataAccessException e) {
			System.out.println("e : " + e + " - SQL QUERY_________" + query);
			return advertiserTrackList;
		} catch (Exception e) {
	...
	public int getAdvertiserView(int advertiserId) {
		String query = "";
		int count = 0;
		try {
			query = "SELECT count(TRACK_ADVERTISER_VIEW_ID) FROM adwit_digitized_ads.t_track_advertiser_view where ADVERTISER_ID=?";
			Object[] params = new Object[] { advertiserId };
	...
	public  int checkSubDomain(int advertiserId) {
		int subDomainId = 0;
		try {
			String query = "SELECT SUBDOMAIN_ID FROM adwit_digitized_ads.t_advertiser where ADVERTISER_ID=?";
			Object[] params = new Object[] { advertiserId };
			subDomainId = jdbcTemplate.queryForObject(query, params, Integer.class);
